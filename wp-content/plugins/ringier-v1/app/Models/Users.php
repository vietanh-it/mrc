...
class Users
{
    ...
    /**
     *  Tạo mới thông tin user trong form_event
     *
     * @param $data
     * @return bool|false|int
     */
    public function inlineFormRegister($data)
    {
        $message = 'Có lỗi xảy ra, vui lòng thử lại.';
        $status = 'error';

        // kiem tra email ton tai
        $query_check_email = "SELECT COUNT(form_register_ID) 
                      FROM $this->_table_form_event
                      WHERE event_name LIKE '" . $data['ten_chuong_trinh'] . "'
                      AND participant_email LIKE '" . $data['email'] . "'";

        $result = intval($this->_wpdb->get_var($query_check_email));

        if ($result > 0) {
            $message = 'Bạn đã đăng ký với email này.';
        } else {
            $data_form_info = array(
                'event_name' => !empty($data['ten_chuong_trinh']) ? $data['ten_chuong_trinh'] : '',
                'participant_name' => !empty($data['ho_ten']) ? $data['ho_ten'] : '',
                'participant_dob' => !empty($data['ngay_sinh']) ? $data['ngay_sinh'] : '',
                'participant_email' => !empty($data['email']) ? $data['email'] : '' ,
                'participant_phone' => !empty($data['so_dien_thoai']) ? $data['so_dien_thoai'] : '',
                'participant_add' => !empty($data['dia_chi']) ? $data['dia_chi'] : '',
                'participant_weddingday' => !empty($data['ngay_cuoi']) ? $data['ngay_cuoi'] : '',
                'extra_follower_name' => !empty($data['ho_ten_nguoi_di_cung']) ? $data['ho_ten_nguoi_di_cung'] : '',
                'extra_follower_phone' => !empty($data['so_dien_thoai_nguoi_di_cung']) ? $data['so_dien_thoai_nguoi_di_cung'] : '',
                'extra_follower_number' => !empty($data['so_luong_nguoi_di_cung']) ? $data['so_luong_nguoi_di_cung'] : '',
                'family_number' => !empty($data['so_luong_thanh_vien_trong_gia_dinh']) ? $data['so_luong_thanh_vien_trong_gia_dinh'] : '',
                'participant_work' => !empty($data['nghe_nghiep']) ? $data['nghe_nghiep'] : '',
                'participant_income' => !empty($data['thu_nhap']) ? $data['thu_nhap'] : '',
                'participant_degree' => !empty($data['hoc_van']) ? $data['hoc_van'] : '',
                'participant_facebook' => !empty($data['link_facebook']) ? $data['link_facebook'] : '',
                'created_at' => current_time('mysql')
            );
            $this->_wpdb->insert($this->_table_form_event, $data_form_info);
            $status = 'success';
