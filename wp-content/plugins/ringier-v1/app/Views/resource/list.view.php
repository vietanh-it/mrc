<?php

get_header();
$list_post = !empty($list_post) ? $list_post : array();

?>
<div class="container">
    <div class="row">
        <h1 class="col-xs-12 col-sm-12 tile-main">resources
            <br> <img src="<?php echo VIEW_URL . '/images/line.png' ?>">
        </h1>
    </div>
</div>
<div class="featured-rs">
    <div class="row">
        <div class="col-xs-12 col-sm-4">
            <div class="box-ft-rs" style="position: relative">
                <a href="#" >
                    <img src="http://www.vietcruises.com/wp-content/uploads/2016/09/1-1472720716-660x0-3818-1473249422-485x360.jpg' ?>">
                </a>
                <a href="#" class="title"> ahsdahsd sahd sajdhs ahdsa </a>
            </div>
        </div>
        <div class="col-xs-12 col-sm-4">
            <div class="box-ft-rs" style="position: relative">
                <a href="#">
                    <img src="http://www.vietcruises.com/wp-content/uploads/2016/09/1-1472720716-660x0-3818-1473249422-485x360.jpg' ?>">
                </a>
                <a href="#" class="title"> ahsdahsd sahd sajdhs ahdsa </a>
            </div>
        </div>
        <div class="col-xs-12 col-sm-4">
            <div class="box-ft-rs" style="position: relative">
                <a href="#" >
                    <img src="http://www.vietcruises.com/wp-content/uploads/2016/09/1-1472720716-660x0-3818-1473249422-485x360.jpg' ?>">
                </a>
                <a href="#" class="title"> ahsdahsd sahd sajdhs ahdsa </a>
            </div>
        </div>
    </div>
</div>

<div class="container">
    <div class="row">
        <div class="col-xs-12 col-sm-10 col-sm-offset-1 main-content">
            <div class="row">
                <div class="col-xs-12 col-sm-12">
                    <h3 class="title-sm">Mekong Princess Team</h3>
                </div>
                <div class="rs-20 box-rs-people">
                    <div class="image">
                        <img src="<?php echo VIEW_URL.'/images/avatar-demo.jpg' ?>" width="100%">
                    </div>
                    <div class="title">David Beckham</div>
                    <div class="job">Captain</div>
                    <div class="desc">
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.
                    </div>
                </div>

                <div class="rs-20 box-rs-people">
                    <div class="image">
                        <img src="<?php echo VIEW_URL.'/images/avatar-demo.jpg' ?>" width="100%">
                    </div>
                    <div class="title">David Beckham</div>
                    <div class="job">Captain</div>
                    <div class="desc">
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.
                    </div>
                </div>

                <div class="rs-20 box-rs-people">
                    <div class="image">
                        <img src="<?php echo VIEW_URL.'/images/avatar-demo.jpg' ?>" width="100%">
                    </div>
                    <div class="title">David Beckham</div>
                    <div class="job">Captain</div>
                    <div class="desc">
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.
                    </div>
                </div>

                <div class="rs-20 box-rs-people">
                    <div class="image">
                        <img src="<?php echo VIEW_URL.'/images/avatar-demo.jpg' ?>" width="100%">
                    </div>
                    <div class="title">David Beckham</div>
                    <div class="job">Captain</div>
                    <div class="desc">
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.
                    </div>
                </div>

                <div class="rs-20 box-rs-people">
                    <div class="image">
                        <img src="<?php echo VIEW_URL.'/images/avatar-demo.jpg' ?>" width="100%">
                    </div>
                    <div class="title">David Beckham</div>
                    <div class="job">Captain</div>
                    <div class="desc">
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.
                    </div>
                </div>
            </div>
            <div class="row box-rs-2">
                <div class="col-xs-12 col-sm-12">
                    <h3 class="title-sm">Onboard Services</h3>
                </div>
                <div class="col-xs-12 col-sm-4">
                    <div class="box-journey">
                        <div class="image">
                            <a href="#" title="SAIGON AND PHNOM PENH">
                                <img src="http://www.vietcruises.com/wp-content/uploads/2016/09/1-1472720716-660x0-3818-1473249422-485x360.jpg" alt="SAIGON AND PHNOM PENH" class="lazy">
                            </a>
                        </div>
                        <div class="desc">
                            <a href="http://local.mrc.com/journeys/saigon-and-phnom-penh" class="title" title="SAIGON AND PHNOM PENH">The Pandaw dining rooms are designed to open up along the sides</a>
                            <p>There can be no tapestry of river life as fascinating and varied as the River Mekong. Indeed there can be no more striking a cultural contrast as...</p>
                        </div>
                    </div>
                </div>
                <div class="col-xs-12 col-sm-4">
                    <div class="box-journey">
                        <div class="image">
                            <a href="#" title="SAIGON AND PHNOM PENH">
                                <img src="http://www.vietcruises.com/wp-content/uploads/2016/09/1-1472720716-660x0-3818-1473249422-485x360.jpg" alt="SAIGON AND PHNOM PENH" class="lazy">
                            </a>
                        </div>
                        <div class="desc">
                            <a href="http://local.mrc.com/journeys/saigon-and-phnom-penh" class="title" title="SAIGON AND PHNOM PENH">The Pandaw dining rooms are designed to open up along the sides</a>
                            <p>There can be no tapestry of river life as fascinating and varied as the River Mekong. Indeed there can be no more striking a cultural contrast as...</p>
                        </div>
                    </div>
                </div>
                <div class="col-xs-12 col-sm-4">
                    <div class="box-journey">
                        <div class="image">
                            <a href="#" title="SAIGON AND PHNOM PENH">
                                <img src="http://www.vietcruises.com/wp-content/uploads/2016/09/1-1472720716-660x0-3818-1473249422-485x360.jpg" alt="SAIGON AND PHNOM PENH" class="lazy">
                            </a>
                        </div>
                        <div class="desc">
                            <a href="http://local.mrc.com/journeys/saigon-and-phnom-penh" class="title" title="SAIGON AND PHNOM PENH">The Pandaw dining rooms are designed to open up along the sides</a>
                            <p>There can be no tapestry of river life as fascinating and varied as the River Mekong. Indeed there can be no more striking a cultural contrast as...</p>
                        </div>
                    </div>
                </div>
            </div>

            <div class="row">
                <div class="col-xs-12 col-sm-12">
                    <h3 class="title-sm">Food and Beverage</h3>
                </div>

                <div class="col-xs-6 col-sm-6">
                    <div class="row">
                        <div class="col-xs-12 col-sm-4">
                            <div class="box-smal-food">
                                <a href="#" title="PHNOM PENH AND SIEM REAP">
                                    <img src="http://local.mrc.com/wp-content/uploads/2016/07/Phnom-Penh-Extension-Header-shutterstock_142484584-500x500.jpg" alt="PHNOM PENH AND SIEM REAP" class="lazy">
                                </a>
                                <a href="#" title="PHNOM PENH AND SIEM REAP">
                                    The Pandaw dining rooms are designed to open up
                                </a>
                            </div>
                        </div>

                        <div class="col-xs-12 col-sm-4">
                            <div class="box-smal-food">
                                <a href="#" title="PHNOM PENH AND SIEM REAP">
                                    <img src="http://local.mrc.com/wp-content/uploads/2016/07/Phnom-Penh-Extension-Header-shutterstock_142484584-500x500.jpg" alt="PHNOM PENH AND SIEM REAP" class="lazy">
                                </a>
                                <a href="#" title="PHNOM PENH AND SIEM REAP">
                                    The Pandaw dining rooms are designed to open up
                                </a>
                            </div>
                        </div>

                        <div class="col-xs-12 col-sm-4">
                            <div class="box-smal-food">
                                <a href="#" title="PHNOM PENH AND SIEM REAP">
                                    <img src="http://local.mrc.com/wp-content/uploads/2016/07/Phnom-Penh-Extension-Header-shutterstock_142484584-500x500.jpg" alt="PHNOM PENH AND SIEM REAP" class="lazy">
                                </a>
                                <a href="#" title="PHNOM PENH AND SIEM REAP">
                                    The Pandaw dining rooms are designed to open up
                                </a>
                            </div>
                        </div>

                        <div class="col-xs-12 col-sm-4">
                            <div class="box-smal-food">
                                <a href="#" title="PHNOM PENH AND SIEM REAP">
                                    <img src="http://local.mrc.com/wp-content/uploads/2016/07/Phnom-Penh-Extension-Header-shutterstock_142484584-500x500.jpg" alt="PHNOM PENH AND SIEM REAP" class="lazy">
                                </a>
                                <a href="#" title="PHNOM PENH AND SIEM REAP">
                                    The Pandaw dining rooms are designed to open up
                                </a>
                            </div>
                        </div>

                        <div class="col-xs-12 col-sm-4">
                            <div class="box-smal-food">
                                <a href="#" title="PHNOM PENH AND SIEM REAP">
                                    <img src="http://local.mrc.com/wp-content/uploads/2016/07/Phnom-Penh-Extension-Header-shutterstock_142484584-500x500.jpg" alt="PHNOM PENH AND SIEM REAP" class="lazy">
                                </a>
                                <a href="#" title="PHNOM PENH AND SIEM REAP">
                                    The Pandaw dining rooms are designed to open up
                                </a>
                            </div>
                        </div>

                        <div class="col-xs-12 col-sm-4">
                            <div class="box-smal-food">
                                <a href="#" title="PHNOM PENH AND SIEM REAP">
                                    <img src="http://local.mrc.com/wp-content/uploads/2016/07/Phnom-Penh-Extension-Header-shutterstock_142484584-500x500.jpg" alt="PHNOM PENH AND SIEM REAP" class="lazy">
                                </a>
                                <a href="#" title="PHNOM PENH AND SIEM REAP">
                                    The Pandaw dining rooms are designed to open up
                                </a>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-xs-6 col-sm-6">
                    <div class="list-galary-2">
                        <div>
                            <a href="http://local.mrc.com/wp-content/uploads/2016/07/Phnom-Penh-Extension-Header-shutterstock_142484584-500x500.jpg" class="fancybox" rel="ship photos"><img src="http://local.mrc.com/wp-content/uploads/2016/07/Phnom-Penh-Extension-Header-shutterstock_142484584-500x500.jpg"></a>
                        </div>
                        <div>
                            <a href="http://local.mrc.com/wp-content/uploads/2016/07/Phnom-Penh-Extension-Header-shutterstock_142484584-500x500.jpg" class="fancybox" rel="ship photos"><img src="http://local.mrc.com/wp-content/uploads/2016/07/Phnom-Penh-Extension-Header-shutterstock_142484584-500x500.jpg"></a>
                        </div>
                        <div>
                            <a href="http://local.mrc.com/wp-content/uploads/2016/07/Phnom-Penh-Extension-Header-shutterstock_142484584-500x500.jpg" class="fancybox" rel="ship photos"><img src="http://local.mrc.com/wp-content/uploads/2016/07/Phnom-Penh-Extension-Header-shutterstock_142484584-500x500.jpg"></a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>


<?php get_footer() ?>
